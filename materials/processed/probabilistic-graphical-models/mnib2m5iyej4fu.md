...
Source: `materials/archive/mnib2m5iyej4fu.pdf`
Duplicate equivalents: `mnib2m5iyej4fu.pdf`
...
3. (2 points) Select all that apply: Which of the following statements about GANs are correct?
    ...
    * ☐ Replacing the DC-GAN loss with the Wasserstein loss empirically ameliorates gradient vanishing.
    1 and 4 are correct.
### Visual Description
Text-only slide with multiple-choice questions. Each question has a list of statements with checkboxes, and the correct answers are indicated below in red text.
---
## Page 3
### Content
10-708 Page 3 of 46
...
### Visual Description
Text-only slide with multiple-choice questions. Each question has a list of statements with checkboxes, and the correct answers are indicated below in red text for questions 4 and 5. Question 6 does not have the correct answer indicated.
---
## Page 4
### Content
10-708 Page 4 of 46
...
### Visual Description
Text-only slide. The page contains only a single sentence in red text.
---
## Page 5
### Content
10-708 Page 5 of 46
...
8. (1 point) True or False: In Noise Contrastive Estimation (NCE), it is generally best to choose the noise distribution to be as far as possible from the data distribution, so that the classifier can easily distinguish data from noise. This significantly improves the sample efficiency of NCE.
    ...
    * False
    False.
### Visual Description
Text-only slide with True/False questions. Each question has radio buttons for True/False, and the correct answer is indicated below in red text.
---
## Page 6
### Content
10-708 Page 6 of 46
...
### Visual Description
Text-only slide with a multiple-choice question about VAE gradient estimation. The question is broken into three scenarios (a, b, c), each with two checkbox options (REINFORCE, Reparameterization trick) and a detailed explanation of the correct answer in red text.
---
## Page 7
### Content
10-708 Page 7 of 46
...
* (d)
* None of the above
c Only as w₁ blocks the back door path
### Visual Description
The slide presents a multiple-choice question about causal graphs and the backdoor criterion. It features "Figure 1", which contains four distinct causal directed acyclic graphs (DAGs) labeled (a), (b), (c), and (d). Each graph shows nodes T (Treatment), Y (Outcome), and various intermediate nodes (W1, W2, W3, V, M, Z) connected by arrows. Some nodes in each graph are highlighted in gray. Below the figure, there are five checkbox options corresponding to the graphs and "None of the above". The correct answer, "c Only as w₁ blocks the back door path", is written in red text below the options.
---
## Page 8
### Content
10-708 Page 8 of 46
...
### Visual Description
The slide contains two multiple-choice questions. Question 11 asks about structures identifiable by the PC algorithm and refers to "Figure 2", which displays four simple causal graphs (a, b, c, d) with nodes and directed edges. Below the figure, there are checkbox options for each graph and "None of the above", with the correct answer "(b) and (d)" in red text. Question 12 asks about the predictor-corrector algorithm for score-based diffusion models, presenting four statements with checkboxes.
## Page 9
### Content
10-708
Page 9 of 46
...
*   The squared norm of the gradient of the energy with respect to the data: $||∇_x E_\theta(x)||^2$.
...
*   The gradient of the log-partition function with respect to parameters: $∇_\theta \log Z(\theta)$.
1 and 2 are correct.
After integration by parts, the score matching loss can be written (up to constants) as
$$
E_{p_{data}} \left[ \left\| \nabla_x \log p_\theta(x) \right\|^2 + \sum_{i=1}^d \partial_{x_i x_i} \log p_\theta(x) \right] = E_{p_{data}} \left[ \left\| \nabla_x E_\theta(x) \right\|^2 - \sum_{i=1}^d \partial_{x_i x_i} E_\theta(x) \right]
$$
...
### Visual Description
A page with two multiple-choice questions (13 and 14) and their options. Question 13 is about the usefulness of the probability flow ODE rewrite in diffusion models, with options related to likelihood evaluation, sample generation, and latent representation finding. Question 14 is about quantities required for Hÿvarinen's original score matching objective for continuous energy-based models, with options related to the squared norm of the gradient of energy, the trace of the Hessian of energy, the log-partition function, and its gradient. The correct answers are indicated. Below the questions, a mathematical equation for the score matching loss is presented, showing its equivalence to terms involving first and second-order derivatives of the energy function.
---
...
*   False
### Visual Description
The page starts with "2 Problems" and "1. Consider the Bayesian Network described in Figure 3". A directed acyclic graph (DAG) is shown, labeled "Figure 3: Bayesian Network Structure", with nodes A, B, C, D, E, F, G, H and directed edges: A->B, B->C, B->D, B->H, C->E, D->E, D->F, D->G, F->E, G->E, H->G. Below the figure, part (a) asks to factorize the joint distribution, and parts (b), (c), (d) ask true/false questions about conditional independence, with the correct answers and brief explanations provided.
---
...
### Visual Description
Continuation of the Bayesian Network problem. Part (e) is a true/false question about conditional independence ($A \perp E | C, D, H$), with the answer and explanation provided. Parts (f) and (g) are fill-in-the-blank questions asking for the nodes in the Markov blanket of C and D, respectively, with the answers provided. Part (h) asks to draw the undirected moralized graph, with an empty box provided for the drawing.
---
...
Page 12 of 46
### Visual Description
The top half of the page is an empty white box, presumably for the drawing of the moralized graph requested on the previous page. The bottom half of the page displays the same Bayesian Network structure (DAG) as on page 10, with nodes A, B, C, D, E, F, G, H and directed edges.
---
...
### Visual Description
The page presents Problem 2, which involves a Markov Chain with 4 states {A, B, C, D}. The transition matrix T is provided. Part (a) asks to draw the transition graph, with an empty box provided. Below the box, the correct transition graph is drawn, showing states A, B, C, D as nodes and directed edges with probabilities: A to B (0.6), A to C (0.4), B to A (0.2), B to B (0.5), B to C (0.3), C to B (0.5), C to D (0.5), D to B (1.0). Part (b) asks if the Markov Chain is irreducible.
---
...
### Visual Description
The top half of the page is an empty white box. The bottom half provides a detailed explanation for why the Markov Chain is irreducible. It lists specific paths and their probabilities from each state (A, B, C, D) to all other states, demonstrating that every state is reachable from every other state.
---
...
### Visual Description
Text-only slide. The slide contains a single paragraph summarizing the conclusion that the Markov chain is irreducible because there is a sequence of transitions with positive probability connecting every pair of states.
---
...
Yes, the Markov Chain is aperiodic. Irreducibility, combined with any self-loop (node B in this case), makes the chain aperiodic. You can also prove $gcd(\{t|p_t(s, s) > 1\} = 1$ for all states $s \in \{A, B, C, D\}$
...
### Visual Description
The page presents parts (c) and (d) of Problem 2. Part (c) asks if the Markov Chain is aperiodic and provides a reason, stating that irreducibility combined with a self-loop (at node B) makes it aperiodic, and also mentions the GCD condition. An empty box is at the top, likely for the answer to (c). Part (d) asks if the Markov Chain has a unique stationary distribution, and if so, to provide it and explain its uniqueness, or if not, to provide multiple stationary distributions.
---
## Page 17
### Content
...
$\begin{cases} \pi_A = 0.2\pi_B \\ \pi_C = 0.38\pi_B \\ \pi_D = 0.19\pi_B \\ \pi_A + \pi_B + \pi_C + \pi_D = 1 \end{cases}$
$\begin{cases} \pi_A = \frac{20}{177} \\ \pi_B = \frac{100}{177} \\ \pi_C = \frac{38}{177} \\ \pi_D = \frac{19}{177} \end{cases}$
...
### Visual Description
The page contains a series of matrix equations and systems of linear equations, followed by the introduction of Problem 3, which describes a graph problem and the initialization step of an algorithm.
---
...
### Visual Description
This page contains text describing an iterative algorithm for shortest paths, including its update rule, and a proof prompt. There are two large blank rectangular areas below the text.
---
...
### Visual Description
The page describes the initialization and update rules for a GNN implementation, including mathematical expressions for min-aggregation and min-plus update.
---
...
### Visual Description
The page introduces Problem 4, discussing Hyvärinen's score matching objective and two modifications. It then defines a "gradient matching" loss function with an integral equation. A large blank rectangular area is present at the bottom.
---
## Page 25
### Content
Expansion. Expanding the square gives
$L_{grad}(\theta) = \int_{\mathbb{R}} \left( (\nabla_x P_{data})^2 - 2\nabla_x P_{data} \nabla_x q_{\theta} + (\nabla_x q_{\theta})^2 \right) dx$
...
### Visual Description
Text and mathematical equations describing the expansion of a loss function, integration by parts for a cross term, and a three-term decomposition.
---

## Page 26
### Content
(b) (2 points) Recall, in the usual Hyvärinen score matching derivation, one ends up with an objective of the form
$J_{SM}(\theta) = E_{x \sim P_{data}} \left[ ||\nabla_x \log q_{\theta}(x)||^2 + 2\text{Tr} (\nabla_x^2 \log q_{\theta}(x)) + ||\nabla_x \log P_{data}(x)||^2 \right],$
Explain carefully why $L_{grad}(\theta)$ cannot be straightforwardly minimized given only i.i.d. samples $x_1, \dots, x_n \sim P_{data}$, whereas $J_{SM}(\theta)$ can. In particular, point out which of the three terms is not easy to estimate from samples for $L_{grad}$, and contrast with $J_{SM}$.
...
where $\phi$ depends only on $q_{\theta}$ and its derivatives (e.g., $\phi = \frac{1}{2}||\nabla_x \log q_{\theta}(x)||^2 + \frac{1}{2}\nabla_x^2 \log q_{\theta}(x)$). As a result, with samples $x_1, \dots, x_n \sim P_{data}$ we can approximate $J_{SM}(\theta)$ via the empirical average $\frac{1}{n}\sum_i \phi(x_i; \theta)$; no knowledge of $P_{data}(x)$ in closed form is needed.
...
$= E_{P_{data}} [s_{\theta}^T B s_{\theta}] - 2 E_{P_{data}} [s_{data}^T B s_{\theta}] + E_{P_{data}} [s_{data}^T B s_{data}].$
...
### Visual Description
Text and mathematical equations showing the expansion of the generalized score matching loss and the integral form of the cross term.
---
...
Since $J_A(\theta)$ is an expectation with respect to $P_{data}$ of a function that we can evaluate at sample points (given $A$ and $q_{\theta}$), we can approximate $J_A(\theta)$ from samples $x_1, \dots, x_n \sim P_{data}$ just as in standard score matching.
...
### Visual Description
Text and mathematical equations explaining the generalized score matching objective and introducing a mixture model for data and model distributions.
---
...
* Upweight the tail region: For radii $||x||_2$ comparable to where the rare component lives (e.g., $||x||_2$ not too far below $R$), $A(x)$ should have larger norm (e.g., $a(||x||_2)$ increasing with $||x||_2$), so that points likely from the rare component contribute strongly to the loss.
...
### Visual Description
Text and mathematical equations discussing properties of a good weighting function $A(x)$ for score matching, followed by the introduction of a new problem comparing GAN losses in a 1D example.
---
...
$M := \frac{1}{2}(p + p_m)$.
### Visual Description
Mathematical definitions of Jensen-Shannon Divergence (JSD) and its components.
---
## Page 33
### Content
...
### Visual Description
The page presents a problem statement for part (b) of an assignment, asking to compute JSD for disjoint supports. Below the problem, there is a large empty white box, likely for a solution or diagram, which is not filled. The bottom part of the page contains text explaining the setup of uniform distributions $p$ and $p_m$ with disjoint supports, followed by calculations for the mixture distribution $M(x)$ in different regions.
---
## Page 34
### Content
Therefore,
...
## Page 35
### Content
(c) (3 points) Let $L_{JS}(m) := \text{JSD}(p||p_m)$ be the generator loss for the JS-based GAN. Compute the derivative $\frac{d}{dm}L_{JS}(m)$ for $|m| > 1$.
...
$L_{JS}(m) = \text{JSD}(p||p_m) = \log 2$,
a constant independent of $m$. Therefore
$\frac{d}{dm}L_{JS}(m) = 0$ for all $|m| > 1$.
### Visual Description
The page presents a problem statement for part (c) of an assignment, asking to compute the derivative of the JS-based GAN generator loss and explain its implications. Below the problem, there is a large empty white box, likely for a solution or diagram, which is not filled. The bottom part of the page contains text stating the value of $L_{JS}(m)$ and its derivative.
---
## Page 36
### Content
...
$W_1(p,p_m) \ge |m|$.
### Visual Description
The page presents a problem statement for part (d) of an assignment, introducing the Wasserstein-1 distance and asking to prove a lower bound for it using specific 1-Lipschitz functions. Below the problem, there is a large empty white box, likely for a solution or diagram, which is not filled.
---
## Page 38
### Content
...
and
$\mathbb{E}_{Y \sim p_m}[f(Y)] = \mathbb{E}[-Y] = -(m + \frac{1}{2})$.
Hence
...
### Visual Description
The page presents a problem statement for part (e) of an assignment, asking to prove an integral identity for the difference of expectations and then use it to derive an upper bound for the Wasserstein-1 distance. Below the problem, there is a large empty white box, likely for a solution or diagram, which is not filled.
---
## Page 40
### Content
...
### Visual Description
The page contains a question (f) asking to show $W_1(p, p_m) = |m|$ by combining with Part (i). Below the question, there is a large empty rectangular box, followed by a short answer in red text.
---
## Page 42
### Content
...
### Visual Description
The page contains a question (g) defining the W-GAN generator loss and asking for its derivative and a comparison with JS-based loss. Below the question, there is a large empty rectangular box. Below the box, the derivatives for $m > 0$ and $m < 0$ are calculated, followed by a note about the derivative at $m = 0$.
---
## Page 43
### Content
